...
try {
  ...
  const mod = await import("../../spal_validator/plutus.json", {
    with: { type: "json" },
  })
...
export class SPALEnforcer {
  ...
  async validate(
    ...
    if (policy.minPayment > 0n) {
      if (request.paymentAmount < policy.minPayment) {
        const unit =
          policy.paymentCurrency.kind === "NativeToken"
            ? "token units"
            : "lovelace"
        return {
          valid: false,
    ...
    for (const utxo of utxos) {
      if (utxo.datum) {
        try {
          ...
          // Skip UTxOs whose datum is not a valid S-PAL policy.
        }
      }
    }
  ...
  async buildValidationTx(
    ...
    const targetUtxo = utxos.find(
      (u: UTxO) =>
        u.txHash === policyUtxo.txHash &&
        u.outputIndex === policyUtxo.outputIndex,
    )
    ...
    if (policy.minPayment > 0n && request.paymentAmount > 0n) {
      ...
      if (policy.paymentCurrency.kind === "NativeToken") {
        ...
        txBuilder = txBuilder.pay.ToAddress(ownerAddress, {
          lovelace: MIN_UTXO_LOVELACE,
          [policy.paymentCurrency.policyId + policy.paymentCurrency.assetName]:
