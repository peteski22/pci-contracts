...
// biome-ignore lint/correctness/noUnusedVariables: reserved for the tx submission step (not yet implemented).
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `✓ Yaci ready - latest block: ${block.number || block.height || "N/A"}`,
        )
        return
      }
    } catch {
...
async function getGenesisWallet(): Promise<{
  address: string
  utxos: unknown[]
}> {
...
function getValidatorInfo(blueprint: Blueprint) {
  ...
  const spendValidator = blueprint.validators.find((v) =>
